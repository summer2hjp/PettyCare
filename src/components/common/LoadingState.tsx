...
export function LoadingState({ lines = 3, card = false, message, className }: LoadingStateProps) {
  if (card) {
    return (
      <div className={cn('mm-card p-5 space-y-4', className)}>
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center py-12 px-4 gap-4', className)}>
      <div className="flex gap-1.5">
        {[0, 1, 2].map(i => (
          <div key={i} className="w-2.5 h-2.5 rounded-full bg-[var(--mm-link)] opacity-30 animate-pulse-soft"
            style={{ animationDelay: `${i * 0.15}s` }} />
        ))}
      </div>
      {message && <p className="text-mm-caption text-[var(--mm-tertiaryLabel)] animate-pulse-soft">{message}</p>}
    </div>
  )
}

...
export default function DailyMoments({ petId = null, pets = [], limit = 12 }: DailyMomentsProps) {
  const { moments, loading, error, refresh } = usePetMoments({ petId, type: 'daily', limit, pets })
    ...
    return (
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3 px-1">
          <DynamicType styleLevel="section" weight={600}>📸 宠物的日常</DynamicType>
        </div>
    ...
    <div className="mb-6">
      ...
      {loading ? (
        ...
        <div className="rounded-xl glass-light p-6 flex flex-col items-center gap-2">
          ...
          <DynamicType styleLevel="caption" className="text-[var(--mm-secondaryLabel)]">还没有日常记录</DynamicType>
        </div>
      ) : (
          ...
          <div ref={scrollRef} className={cn('flex gap-3 overflow-x-auto pb-1', selecting ? '' : 'scrollbar-none snap-x snap-mandatory')}>
            {moments.map(moment => (
              <div key={moment.id} className="relative shrink-0">
                ...
                <MomentCard moment={moment} type="daily" onClick={selecting ? () => toggleSelect(moment.id) : () => {}} />
              </div>
            ))}
          </div>

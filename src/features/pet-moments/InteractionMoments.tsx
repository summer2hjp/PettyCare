...
interface InteractionMomentsProps {
  petId?: string | null
  pets?: Pet[]
  limit?: number
}

export default function InteractionMoments({ petId = null, pets = [], limit = 8 }: InteractionMomentsProps) {
  const { moments, loading, error, refresh } = usePetMoments({ petId, type: 'interaction', limit, pets })
    ...
    return (
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3 px-1">
          <DynamicType styleLevel="section" weight={600}>💕 互动瞬间</DynamicType>
        </div>
  ...
  return (
    <div className="mb-6">
      ...
      {loading ? (
        <GlassMomentSkeleton count={4} aspectRatio="portrait" />
      ) : moments.length === 0 ? (
        <div className="rounded-xl glass-light p-6 flex flex-col items-center gap-2">
          ...
          <DynamicType styleLevel="caption" className="text-[var(--mm-secondaryLabel)]">还没有互动记录</DynamicType>
        </div>
      ) : (
        <div className="relative">
          <div ref={scrollRef} className={cn('flex gap-3 overflow-x-auto pb-1', selecting ? '' : 'scrollbar-none snap-x snap-mandatory')}>
            {moments.map(moment => (
              <div key={moment.id} className="relative shrink-0">
                ...
                <MomentCard moment={moment} type="interaction" onClick={selecting ? () => toggleSelect(moment.id) : () => {}} />
              </div>
            ))}
          </div>

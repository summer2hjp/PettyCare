...
|---|-----------|--------|--------|--------|
| D1.1 | Dashboard 侧边栏标签 | 标签 `Dashboard` 在侧边栏第一位，图标 LayoutDashboard，点击后高亮蓝色背景 | | |
| D1.2 | 页面标题 | 显示 "Welcome back 👋" | | |
| D1.3 | Quick Actions | 5 个按钮水平排列：Add Pet / Log Feeding / Log Vaccination / Schedule Visit / Add Vet Visit，AppleCard 容器，footnote 字重 600 | | |
| D1.4 | Health Overview Section | 标题 "Health Overview"，副标题宠物数，右上角 "View All"（蓝色 + ChevronRight） | | |
| D1.5 | Health Score 卡片 | AppleProgressRing + 百分比 + "Health Score" + 状态文字（环色匹配） | | |
| D1.6 | 疫苗卡片 | Syringe 图标 + 计数 + "Vaccinations due"，title2 字重 700 | | |
| D1.7 | 就诊卡片 | Stethoscope 图标 + 日期 + 兽医名 + "Last visit" | | |
| D1.8 | 用药卡片 | Pill 图标 + 计数 + "Active medications" | | |
| D1.9 | Activity Section | 标题 "Activity" + "View All"，4 张统计卡片（steps/distance/duration/calories） | | |
| D1.10 | 统计卡片样式 | 圆角图标容器（10% opacity）+ 标签 + 数值 + 单位 | | |
| D1.11 | 趋势卡片 | TrendingUp/Down 图标 + "Weekly trend" + 百分比 | | |
| D1.12 | Feeding Schedule Section | 标题 + "View All"，餐食时间线卡片 | | |
| D1.13 | 喂养卡片状态 | 已完成（绿色对勾）/ 下一餐（蓝色边框 + 倒计时）/ 待处理（灰色） | | |
| D1.14 | Upcoming Events Section | 标题 + "View All"，事件卡片 | | |
| D1.15 | 紧急度标签 | "Tomorrow"（蓝色）/ "In 3 days"（橙色）/ "Next week"（绿色）/ "Later"（灰色） | | |
| D1.16 | Recent Activity Section | 标题，无 "View All" | | |
| D1.17 | 动态卡片 | 彩色圆形图标（按类型）+ 描述 + 详情 + 相对时间 | | |
| D1.18 | Insights Section | 标题，无 "View All" | | |
| D1.19 | 洞察卡片 | Trend（柱状图）/ Tip（提示）/ Comparison（对比） | | |
| D1.20 | 水平滚动 | 每个 Section `snap-x snap-mandatory`，`scrollbar-none`，卡片 gap-3 | | |
| D1.21 | "View All" 按钮 | 4 个 Section 存在 "View All"，`text-apple-blue` | | |
| D1.22 | 卡片 hover | hoverable 卡片 hover 时 elevation 提升，`transition-all duration-300` | | |
| D1.23 | 深色模式 | CSS 变量加载正常（`--apple-systemBackground` 等），卡片使用语义色 | | |

### 3.2 Pets — 宠物列表

| # | Test Path | 期望值 | 实际值 | Result |
|---|-----------|--------|--------|--------|
| P1.1 | Pets 侧边栏标签 | 标签 `Pets`，图标 PawPrint，点击后高亮 | | |
| P1.2 | 页面标题 | 显示宠物的 icon 和 "Pets" | | |
| P1.3 | 搜索框 | 搜索输入框，placeholder 包含 "Search" | | |
| P1.4 | 物种筛选 | SegmentedControl：All / Dogs / Cats / Other | | |
| P1.5 | 宠物卡片网格 | 每个宠物显示 avatar + 名称 + 品种 + 年龄，"View Profile" 按钮 | | |
| P1.6 | Add Pet 按钮 | 页面顶部 "Add Pet" 按钮（Plus 图标） | | |
...
| # | Test Path | 期望值 | 实际值 | Result |
|---|-----------|--------|--------|--------|
| D2.1 | Dashboard → Add Pet | 跳转表单页，侧边栏 **Pets** 高亮，表单含 Name/Breed/Species/Gender/BirthDate/Weight 等字段 | | |
| D2.2 | Dashboard → Log Feeding | 跳转 Feeding 页，侧边栏 **Feeding** 高亮 | | |
| D2.3 | Dashboard → Log Vaccination | 跳转 Health 页，侧边栏 **Health** 高亮 | | |
| D2.4 | Dashboard → Schedule Visit | 跳转 Appointments 页，侧边栏 **Appointments** 高亮 | | |
| D2.5 | Dashboard → View All (Health) | 跳转 Health 页，侧边栏 **Health** 高亮 | | |
| D2.6 | Dashboard → View All (Activity) | 跳转 Activity 页，侧边栏 **Activity** 高亮 | | |
| D2.7 | Dashboard → View All (Feeding) | 跳转 Feeding 页，侧边栏 **Feeding** 高亮 | | |
| D2.8 | Dashboard → View All (Events) | 跳转 Appointments 页，侧边栏 **Appointments** 高亮 | | |

### 4.2 Pets — 表单操作

| # | Test Path | 期望值 | 实际值 | Result |
|---|-----------|--------|--------|--------|
| P2.1 | Pets → Add Pet → 表单字段 | Name placeholder "Pet's name"；Breed placeholder "e.g. Golden Retriever"；DOB date 类型；Weight number step 0.1 | | |
| P2.2 | Pets → Add Pet → 表单验证 | Name/Breed/DOB 空 → 显示 "Required"；Weight ≤ 0 → "Must be positive" | | |
| P2.3 | Pets → Add Pet → 提交成功 | 填写有效数据 → Save → 跳转宠物列表 → 新宠物出现在列表 | | |
| P2.4 | Pets → Add Pet → 取消 | 点击 Cancel → 返回上一页，数据不变 | | |
| P2.5 | Pets → 宠物卡片 → View Profile | 点击 "View Profile" → 跳转 PetDetailPage，显示宠物完整信息 | | |
...
| # | Test Path | 期望值 | 实际值 | Result |
|---|-----------|--------|--------|--------|
| 3.1 | Pets → Add Pet → Save → Pets 列表 → 点击新宠物 → Pet Profile → 编辑 | 编辑表单预填已保存数据；修改名称 → Save → 名称更新成功 | | |
| 3.2 | Pets → 宠物列表 → View Profile → Pet Detail → 删除 | 删除确认弹框 → 确认 → 宠物从列表移除 | | |
...
| # | Test Path | 期望值 | 实际值 | Result |
|---|-----------|--------|--------|--------|
| 4.1 | 宠物名称一致性 | Dashboard（健康/喂养/事件/动态）/ Pets 列表 / Pet Profile 中宠物名称保持一致 | | |
| 4.2 | 健康数据同步 | Health Page 中的疫苗接种状态同步到 Dashboard 的 "Vaccinations due" 和 "Upcoming Events" | | |
...
| 4.5 | 宠物数量一致性 | 所有宠物计数位置（Dashboard 副标题、Pets 列表计数）显示相同数量 | | |
| 4.6 | CRUD 闭环 | 新增宠物 → 出现在 Dashboard / Pets 列表 → 编辑 → 名称更新 → 删除 → 所有位置移除 | | |
| 4.7 | 深色模式全局 | Settings 切换深色模式 → Dashboard / Pets / Health 等所有页面同步切换 | | |
